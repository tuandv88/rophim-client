...
import { sortFieldOptions, yearOptions } from "../../../constants/filters";
...
export type TaxonomyFilterValues = z.infer<typeof filterSchema>;
...
interface TaxonomyFilterSidebarProps {
  ...
  showCountry: boolean;
  showYear: boolean;
  onApply: (values: TaxonomyFilterValues) => void;
  onReset: () => void;
}

export function TaxonomyFilterSidebar({
  values,
  categories,
  countries,
  showCategory,
  ...
  onApply,
  onReset,
}: TaxonomyFilterSidebarProps) {
  const { control, register, handleSubmit, reset } = useForm<TaxonomyFilterValues>({
    ...
    <div className="space-y-4 lg:sticky lg:top-24">
      ...
      <aside className={cn(
        ...
        <form className="space-y-5" onSubmit={handleSubmit((data) => {
          ...
          {showCategory ? <MultiSelectField control={control} name="category" label="Thể loại" items={categories} /> : null}
          {showCountry ? <MultiSelectField control={control} name="country" label="Quốc gia" items={countries} /> : null}
...
type ControlType = ReturnType<typeof useForm<TaxonomyFilterValues>>["control"];

function MultiSelectField({ control, name, label, items }: MultiSelectFieldProps) {
  return (
    <Controller
      control={control}
      name={name}
      render={({ field }) => (
        <fieldset className="space-y-2">
          <legend className="text-sm font-semibold text-white/60">{label}</legend>
